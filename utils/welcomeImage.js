...
async function generateWelcomeImage({ username, avatarUrl, memberCount, welcomeColor, descColor, descMessage, bgPath, bgUrl }) {
    const canvas = createCanvas(W, H);
    const ctx    = canvas.getContext('2d');

    // ── Fondo (local tiene prioridad sobre URL) ────────────────────────────
    const hasBg = (bgPath && fs.existsSync(bgPath)) || bgUrl;
    if (hasBg) {
        try {
            const bg = bgPath && fs.existsSync(bgPath)
