    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (sub === 'probar') {
                ...
                // ── Si hay premium configurado, simula el flujo real ──
                let premiumEnviada = false;
                if (cfg?.premium?.enabled && cfg.premium.bgPath && fs.existsSync(cfg.premium.bgPath)) {
                    try {
                        const { generateWelcomeImage } = require('../utils/welcomeImage');
                        const previewMsg = (cfg.premium.descMessage || '')
                        ...
                        const buf = await generateWelcomeImage({
                            ...
                            welcomeColor: cfg.premium.welcomeColor,
                            descColor:    cfg.premium.descColor,
                            descMessage:  previewMsg,
                            bgPath:       cfg.premium.bgPath,
                        });
                        const file = new AttachmentBuilder(buf, { name: 'welcome-premium.png' });
                        const premMsg = await targetChannel.send({
                            content: `<a:holi:1385228499438145568> ¡Hola ${interaction.user}!`,
                            files:   [file],
                        });
                        premiumEnviada = true;

                        // Mismo temporizador que en guildMemberAdd
                        if (cfg.premium.previewMinutes && Number.isFinite(cfg.premium.previewMinutes)) {
                            setTimeout(async () => {
                                try {
                                    await premMsg.delete().catch(() => {});
                                    if (!cfg?.messageDisabled) {
                                        await targetChannel.send({
                                            content: `<a:holi:1385228499438145568> ¡Hola ${interaction.user}!`,
                                            embeds:  [embed],
                                        }).catch(() => {});
                                    }
                                } catch (e) { console.error('Error revertiendo premium en probar:', e); }
                            }, cfg.premium.previewMinutes * 60_000);
                        }
                    } catch (imgErr) {
                        console.error('Error generando premium en probar:', imgErr);
                    }
                }

                // Si no hay premium, o el premium falló, manda el embed normal (a no ser que esté desactivado)
                if (!premiumEnviada && !cfg?.messageDisabled) {
                    await targetChannel.send({
                        content: `<a:holi:1385228499438145568> ¡Hola ${interaction.user}!`,
                        embeds: [embed]
                    });
                ...
                const partes = [];
                if (premiumEnviada) {
                    partes.push(cfg.premium.previewMinutes
                        ? `imagen **premium** (visible ${cfg.premium.previewMinutes} min)`
                        : 'imagen **premium** (permanente)');
                }
                if (!premiumEnviada && !cfg?.messageDisabled) partes.push('embed **normal**');
                if (partes.length === 0) partes.push('⚠️ Nada — el mensaje normal está desactivado y no hay premium');

                const msg = targetChannel.id !== interaction.channel.id
                    ? `✅ Prueba enviada en ${targetChannel}: ${partes.join(' + ')}.`
                    : `✅ Prueba enviada: ${partes.join(' + ')}.`;
