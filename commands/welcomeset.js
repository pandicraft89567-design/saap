    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub
            ...
            .addStringOption(opt => opt
                ...
                .setDescription('URL de la imagen de fondo (PNG, JPG, GIF)')
                .setRequired(true))
            .addStringOption(opt => opt
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (sub === 'probar') {
                ...
                // ── Si hay premium configurado, mostrar la versión premium ──
                if (cfg?.premium?.enabled) {
                    const prem = cfg.premium;
                    ...
                    let imgBuffer;
                    try {
                        imgBuffer = await generateWelcomeImage({
                        ...
                        console.error('Error generando preview premium en probar:', imgErr);
                        return await interaction.editReply({ content: '❌ No pude generar la imagen premium. Revisa la URL del fondo.' });
                    }

                    const file = new AttachmentBuilder(imgBuffer, { name: 'bienvenida-prueba.png' });
                    await targetChannel.send({
                        content: `<a:holi:1385228499438145568> ¡Hola ${interaction.user}! *(prueba premium)*`,
                        files: [file]
                    });

                    return await interaction.editReply({ content: msg + '\n✨ Se usó tu configuración **premium**.' });
                }
            ...
            if (sub === 'premium') {
                ...
                const imagenFondo  = interaction.options.getString('imagen_fondo').trim();
                ...
                try {
                    ...
                    return await interaction.editReply({ content: '❌ No pude cargar la imagen de fondo. Revisa que la URL sea válida y accesible.' });
                }
