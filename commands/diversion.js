...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub
            ...
            .addUserOption(opt => opt.setName('usuario')
                .setNameLocalizations({ 'en-US': 'user', 'en-GB': 'user' })
                .setDescription('¿A quién quieres castigar?')
                .setDescriptionLocalizations({ 'en-US': 'Who do you want to punish?', 'en-GB': 'Who do you want to punish?' })
                .setRequired(true))
    ...
    async execute(interaction) {
        ...
        if (sub === 'castigar') {
            ...
            if (target.id === PROTECTED_USER_ID)
                return interaction.reply({ content: '<:kokoro:1385223047207850024> No, ella es mi novia así que no puedes hacer eso...', flags: 64 });
            if (target.id === author.id)
                return interaction.reply({ content: '🤔 ¿Castigarte a ti mismo? Eso es un poco masoquista...', flags: 64 });

            await interaction.deferReply();

            const tipo   = interaction.options.getString('tipo') || TIPOS[Math.floor(Math.random() * TIPOS.length)];
            const prompt = `Eres Soledad, un bot de Discord tsundere. Describe en UNA oración cómica cómo "${author.username}" castiga a "${target.username}" con un ${tipo} de anime. Estilo dramático y gracioso. Sin emojis al inicio.`;
            const [gifUrl, aiText] = await Promise.all([fetchAnimeGif(tipo), generateAIMessage(prompt, 100)]);

            const tipoEmoji = { slap: '👋', bonk: '🔨', kick: '👢' }[tipo];
            const fallback  = `**${author.username}** le da un ${tipo} épico a **${target.username}**. ¡Que sirva de lección!`;

            const embed = new EmbedBuilder()
                .setColor('#FF6B35')
                .setTitle(`${tipoEmoji} ¡Castigo!`)
                .setDescription(aiText || fallback)
                .setFooter({ text: `${author.username} castigó a ${target.username}` })
                .setTimestamp();
