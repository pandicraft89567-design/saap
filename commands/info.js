...
module.exports = {
    ...
    async execute(interaction) {
        ...
        // ── Búsqueda de un comando específico ─────────────────────────────
        if (input) {
        ...
        // ── Paginación por categorías con botones ─────────────────────────
        const categories = Object.entries(ALL_COMMANDS);
        const pages      = [];
        const total      = Object.keys(COMMAND_LOOKUP).length;

        for (const [category, cmds] of categories) {
            const seen       = new Set();
            const uniqueCmds = cmds.filter(c => seen.has(c.name) ? false : seen.add(c.name));

            // Dividir en chunks si la descripción es muy larga (límite Discord: 4096)
            const lines = uniqueCmds.map(cmd =>
                `**${cmd.name}**\n┣ 📝 ${cmd.desc}\n┗ ⌨️ \`${cmd.usage}\``
            );
            ...
            chunks.forEach((chunk, i) => {
                pages.push({
                    label: chunks.length > 1 ? `${category} (${i + 1}/${chunks.length})` : category,
                    embed: new EmbedBuilder()
                        .setTitle(chunks.length > 1 ? `${category} — parte ${i + 1}` : category)
                        .setColor('#7289DA')
        ...
        pages[pages.length - 1].embed
            .setFooter({ text: `[ ] = obligatorio  •  ( ) = opcional  •  Total: ${total} comandos` })
            .setTimestamp();
        ...
        collector.on('collect', async i => {
            ...
            await i.update({ embeds: [pages[page].embed], components: [buildRow(page)] });
        });

        collector.on('end', () => {
            const disabledRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('info_prev').setLabel('◀ Anterior').setStyle(ButtonStyle.Secondary).setDisabled(true),
                new ButtonBuilder().setCustomId('info_page').setLabel(`${page + 1} / ${pages.length}`).setStyle(ButtonStyle.Primary).setDisabled(true),
                new ButtonBuilder().setCustomId('info_next').setLabel('Siguiente ▶').setStyle(ButtonStyle.Secondary).setDisabled(true),
            );
