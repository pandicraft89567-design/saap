...
// ════════════════════════════════════════════════════════════════════
// CANALES AUTO (los típicos de Discord)
// ════════════════════════════════════════════════════════════════════
const AUTO_CHANNELS = [
    'bienvenida', 'reglas',     'anuncios',
    'general',    'chat',       'memes',
    'imagenes',   'musica',     'juegos',
    'bots',       'ayuda',      'sugerencias',
];
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (sub === 'auto' && !group) {
            ...
            for (const nombreRaw of AUTO_CHANNELS) {
            ...
            const embed = new EmbedBuilder()
                ...
                .setTitle(`✅ Canales auto creados (${creados.length}/${AUTO_CHANNELS.length})`)
                .setDescription(creados.length ? creados.join(' ') : '*Ninguno se pudo crear.*')
                .setTimestamp();

            if (fallidos.length) embed.addFields({ name: '❌ Fallidos', value: fallidos.join('\n').slice(0, 1024) });
