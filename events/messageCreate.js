...
module.exports = {
    ...
    async execute(message) {
        ...
        // Solo generar cita si empieza con "frase" (ej: @soledad frase <texto>)
        const FRASE_RE = /^frase\s+(.+)/is;
        const fraseMatch = afterMention.match(FRASE_RE);
        const quoteText = fraseMatch ? fraseMatch[1].trim() : null;
        ...
        if (quoteText && quoteText.length > 0) {
            try {
                ...
                const avatarURL = message.author.displayAvatarURL({ extension: 'png', size: 128, forceStatic: true });
                const displayName = message.member?.displayName || message.author.username;

                const imageBuffer = await generateQuoteImage(quoteText, displayName, avatarURL, 'dark');
                ...
                // Cachear datos para regenerar al cambiar estilo
