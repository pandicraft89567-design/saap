        ...
        if (detected && automodOn) {
            try {
                ...
                if (logChannelId) {
                    ...
                    if (logCh) {
                        const msgPreview = message.content.length > 80
                            ? message.content.slice(0, 80) + '…'
                            : message.content;

                        const logEmbed = new EmbedBuilder()
                            .setColor(actionText ? '#ff4757' : '#ff6b35')
                            ...
                            .addFields(
                                ...
                                { name: '🎯 Acción aplicada', value: actionText ?? '⚠️ Solo advertencia — sin sanción configurada', inline: false },
                                { name: '💬 Mensaje eliminado', value: `\`${msgPreview}\``, inline: false }
                            )
                            .setFooter({ text: 'Soledad ❣ • Auto-Moderación' })
                            .setTimestamp();
                        ...
                        if (!actionText) {
                            const selectMenu = new ActionRowBuilder().addComponents(
                                new StringSelectMenuBuilder()
                                    .setCustomId(`am_sel_${message.author.id}`)
                                    .setPlaceholder('⚖️ Aplicar sanción manual...')
                                    .addOptions([
                                        { label: 'Silenciar (timeout)', description: 'Elegir duración del silencio', value: 'silenciar', emoji: '🔇' },
                                        { label: 'Expulsar', description: 'Expulsar del servidor', value: 'expulsar', emoji: '👢' },
                                        { label: 'Banear', description: 'Banear del servidor', value: 'banear', emoji: '🔨' },
                                    ])
                            );
                            await logCh.send({ embeds: [logEmbed], components: [selectMenu] }).catch(() => {});
                        } else {
