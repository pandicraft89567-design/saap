...
module.exports = {
    ...
    async execute(member) {
        ...
        try {
            ...
            // Si el mensaje normal está desactivado y no hay premium, no enviar nada
            if (customConfig?.messageDisabled && !customConfig?.premium?.enabled) return;
            ...
            if (customConfig?.premium?.enabled) {
                ...
                try {
                    ...
                    const premiumMsg = await channel.send({
                        content: `<a:holi:1385228499438145568> ¡Hola ${member}!`,
                        files:   [file],
                    });

                    // ── Reversión automática a mensaje normal ──────────────
                    if (prem.previewMinutes && Number.isFinite(prem.previewMinutes)) {
                        const ms = prem.previewMinutes * 60_000;
                        setTimeout(async () => {
                            try {
                                await premiumMsg.delete().catch(() => {});
                                await channel.send({
                                    content: `<a:holi:1385228499438145568> ¡Hola ${member}!`,
                                    embeds:  [welcomeEmbed],
                                }).catch(() => {});
                            } catch (revErr) {
                                console.error('Error revirtiendo a mensaje normal:', revErr);
                            }
                        }, ms);
                    }
            ...
            if (customConfig?.messageDisabled) return;
